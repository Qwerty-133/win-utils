"""The core functionality for handling key presses."""
...
def toggle_enabled():
    """Toggle the enabled state, and notify the user."""
    State.enabled = not State.enabled
    status = "enabled" if State.enabled else "disabled"
    toast.show_toast(
        "Fn-lock toggled.", f"Fn-lock is currently {status}.", toast.get_icon(ICON_NAME)
    )


def handle_events(e):
    """
    Handle keypresses and releases to emulate fn-lock behaviour.
